...
class forward_jet_analysis(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        if not re.search(data_pattern, dataset):
            ...
            gen_tau = gen[((abs(gen.pdgId)==15) & ((gen.statusFlags & (1 << 13)) > 0) & ((gen.statusFlags & 1) > 0) )]

            prompt_tau = tau[match(tau, gen_tau, deltaRCut=0.4)]
            fake_tau = tau[~match(tau, gen_tau, deltaRCut=0.4)]
        ...
        for var in variations:
            ...
            BL = sel.dilep_baseline(SS=False, omit=['N_fwd>0', 'N_central>2'], Zsel='offZ')  # offZ not in effect if DY is true
            if not re.search(data_pattern, dataset):
                BL = add_conversion_req(dataset, BL)

            BL_SS = sel.dilep_baseline(SS=True, omit=['N_fwd>0', 'N_central>2'], Zsel='offZ')  # offZ not in effect if DY is true
            #
            ...
            output['N_tau'].fill(
                ...
                systematic = var_name,
                n1 = ak.num(tau)[BL],
                n2 = ak.num(prompt_tau)[BL],
                weight=weight.weight()[BL],
            )

            output['N_tau_SS'].fill(
                dataset=dataset,
                systematic = var_name,
                n1 = ak.num(tau)[BL_SS],
                n2 = ak.num(prompt_tau)[BL_SS],
                weight=weight.weight()[BL_SS],
            )
...
if __name__ == '__main__':
    ...
    for sample in sample_list:
        ...
        from processor.default_accumulators import systematic_axis, eft_axis, charge_axis, n_ele_axis, eta_axis, delta_eta_axis, pt_axis, delta_phi_axis, n1_axis, n2_axis
        desired_output.update({
            "N_tau": hist.Hist("Counts", dataset_axis, systematic_axis, n1_axis, n2_axis),
            "N_tau_SS": hist.Hist("Counts", dataset_axis, systematic_axis, n1_axis, n2_axis),
           })
        ...
        cache_name = f'tau_analysis_{sample}_{year}{era}'
    ...
    output_scaled = get_merged_output('tau_analysis', str(year), select_datasets=processes)
