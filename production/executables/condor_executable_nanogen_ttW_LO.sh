...
function edit_psets {
    ...
    # nanogen
    echo "process.RandomNumberGeneratorService.externalLHEProducer.initialSeed = $seed" >> $nanocfg
    echo "process.externalLHEProducer.args = [\"$gridpack\"]" >> $nanocfg
    echo "process.externalLHEProducer.nEvents = $nevents" >> $nanocfg
    echo "process.maxEvents.input = $nevents" >> $nanocfg
    echo "process.source.firstLuminosityBlock = cms.untracked.uint32($seed)" >> $nanocfg
}
...
nanocfg="psets/nanogen/ttW_EFT_LO_cfg.py"
...
setup_cmssw CMSSW_10_6_19 slc7_amd64_gcc820 
cmsRun $nanocfg
CMSRUN_STATUS=$?
