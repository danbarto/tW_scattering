...
def getWeightName( reweight ):
    name = "_".join( [ ("%s_%8.6f"%( reweight[int(2*i)], reweight[int(2*i+1)] )).rstrip('0') for i in range(int(len(reweight)/2)) ] ) 
...
def make_reweight_card( filename, reweights, referencepoint, order_dict ):
    ...
    with open(filename, "w") as out_file:
        ...
        for reweight in reweights:
            ...
            for i in range(int(len(reweight)/2)):
...
def make_reweight_pkl( filename, reweights, referencepoint, order_dict ):
    #import pickle
    f_out = filename.replace('.dat','.yaml')
    rw_dict = {}
    ...
    rw_dict_sorted = {k: v for k, v in sorted(rw_dict.items(), key=lambda item: item[1])}
    
    dump_yaml(rw_dict_sorted, f_out)
...
#    pickle.dump(rw_dict, file(pklfilename,'w')) 
    print("Written yaml file for enumeration (%i weights):"%len(reweights), f_out)
    return rw_dict_sorted
...
if len(args.couplings)>0 and isA( args.couplings[0], int ):
    ...
    for order_ in range(0, order+1):
        for comb in combinations_with_replacement(list(range(len(vars))), order_):
...
if not os.path.exists( args.filename ) or args.overwrite:
    ...
    rw_dict = make_reweight_pkl( args.filename, param_points, referencepoint, order_dict )
