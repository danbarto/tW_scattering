...
from analysis.Tools.objects import Collections, getNonPromptFromFlavour, getChargeFlips, prompt, nonprompt, choose, cross, delta_r, delta_r2, match, nonprompt_no_conv, external_conversion
...
class trilep_analysis(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        mu_t     = Collections(ev, "Muon", "tightTTH", year=self.year).get()
        mu_f     = Collections(ev, "Muon", "fakeableTTH", year=self.year).get()
        
        mu_v_mask = Collections(ev, "Muon", "vetoTTH", year=self.year)
        mu_f_mask = Collections(ev, "Muon", "fakeableTTH", year=self.year)
        mu_t_mask = Collections(ev, "Muon", "tightTTH", year=self.year)
        ...
        el_t        = Collections(ev, "Electron", "tightTTH", year=self.year).get()
        el_f        = Collections(ev, "Electron", "fakeableTTH", year=self.year).get()

        el_v_mask = Collections(ev, "Electron", "vetoTTH", year=self.year)
        el_f_mask = Collections(ev, "Electron", "fakeableTTH", year=self.year)
        el_t_mask = Collections(ev, "Electron", "tightTTH", year=self.year)
