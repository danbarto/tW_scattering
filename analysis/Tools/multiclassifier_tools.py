...
def load_onnx_model(version='v8'):
    model = rt.InferenceSession(os.path.expandvars('data/networks/weights_%s.onnx'%version))
    scaler = joblib.load(os.path.expandvars('data/networks/scaler_%s.joblib'%version))
    return model, scaler
...
def dump_onnx_model(model, version='v8'):
    ...
    onnx.save(onnx_model, os.path.expandvars('data/networks/weights_%s.onnx'%version))
    return True
