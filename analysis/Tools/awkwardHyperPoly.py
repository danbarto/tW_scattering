...
class HyperPoly:
    ...
    def eval( self, coefficients, point ):
        ...
        point_corr = np.array([ np.prod([ (point[elem] - self.ref_point[elem]) for elem in self.combination[n]]) for n in range(self.ndof) ])
        bc_array = ak.broadcast_arrays(np.array(point_corr), ak.to_numpy(coefficients))
        return ak.sum(bc_array[0]*bc_array[1], axis=1)
