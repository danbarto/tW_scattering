...
from analysis.Tools.helpers import get_four_vec_fromPtEtaPhiM
...
def getMET(ev, pt_var='pt'):
    ...
    if pt_var == 'pt_nom': pt_var = 'pt'  # pt_nom only exists for jets (applies JER smearing, which we do not propagate to MET)
    return get_four_vec_fromPtEtaPhiM(
        ev.MET,
        getattr(ev.MET, pt_var.replace('pt', 'T1_pt')),
        zeros,
        getattr(ev.MET, pt_var.replace('pt', 'T1_phi')),
        zeros,
        copy=False,
    )
